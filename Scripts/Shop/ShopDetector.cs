...
public class ShopDetector : MonoBehaviour {
	...
	void UpgradeWeapon(WeaponBase weaponBase, ShopType upgradeType) {
		switch(upgradeType) {
			...
			case ShopType.UPGRADE_RANGE:
				weaponBase.upgradeRange++;
				break;		
			...
			if(hit.transform.tag == "Shop") {
				...
				if(shopType == ShopType.AMMO) {
					...
					if(upgraded < 10) {
						...
						isPurchasable = false;
						shopText.text = "MAXED AMMO!";
					}
				}

				else if(shopType == ShopType.UPGRADE_RANGE) {
					int upgraded = weaponBase.upgradeRange;
				...
				if(isPurchasable && Input.GetKeyDown(KeyCode.F)) {
					...
					if(fund < shopPrice) {
						...
						if(shopType == ShopType.AMMO) {
							weaponBase.bulletsLeft = weaponBase.startBullets + weaponBase.bulletsPerMag + ((int) weaponBase.upgradeMaxAmmoFactor * weaponBase.upgradeMaxAmmo);
							...
							if(weaponBase.upgradeRange >= 10) {
								wasPurchased = false;
								PrintWarning("Your weapon is fully upgraded.");
							}
							else {
								UpgradeWeapon(weaponBase, ShopType.UPGRADE_RANGE);								
							}
						}
